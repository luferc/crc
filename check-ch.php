...
<div id="content">
    <div class="pagename">
<?php
echo strtoupper("update item: ").$_POST['ch'];
?>
    </div>
</div>
<ul>
...
$receptores = mysql_query("SELECT * FROM espectro WHERE CH='$_POST[ch]'");
...
while ($row=mysql_fetch_Array($receptores)) // recorre los clientes uno por uno hasta el fin de la tabla
{
	print '<tr class="indicador">'
		  ...
		  .'<td>'.$row['level'] .'</td>'
		  .'<td>'.$row['label'] .'</td>'
		  .'<td><a href="updates-senal.php?md='.$row['id_espectro'].'">Modificar</a></td>'// en este ejemplo para simplificar se envian los parametros por get utilizando un href
		  .'</tr>';
}
print '</table>';
?>
